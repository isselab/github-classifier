...
class RepositoryDataset(Dataset):
    def __init__(self, directory, label_list=None):
        ...
        for g, graph in enumerate(self.graph_dir):
            try:
                ...
                # create graph = tuple of edges and node features, type: Data object
    ...
    def __len__(self):
        size = len(self.graph_names) #maybe change this to list?
        return size
        ...
        for g,graph in enumerate(self.graph_dir):
            try:
                if f'{graph_name}_nodefeatures' in graph:
                    ...
                    self.x = torch.Tensor(np.array(node_features))  # convert DataFrame object
    ...
    def convert_labeled_graphs(labels, directory):
